...
1. On a fresh developer host, run `./scripts/doctor.sh contributor`; it names the exact missing
   tool and the command that installs it. Follow
   [`docs/runbooks/fresh-linux-setup.md`](docs/runbooks/fresh-linux-setup.md) for the reviewed setup path.
2. Resolve every reason the doctor reports, then run `just setup` to hydrate the committed Python and npm locks.
...
The supported developer host is Ubuntu 26.04 on x86_64 (R71). The Fedora developer bootstrap and its
disposable two-media Workstation acceptance proof were retired with that decision, so there is no
longer a manual host-acceptance gate: `./scripts/doctor.sh` is the host contract, and CI is the
acceptance evidence.
