...
> "rule-heuristic-1.2"`; S-ing-2 shipped `"rule-heuristic-1"`, and the pin has moved once per
> score-changing edit since — `1.1` for the `"audit programme"` matchers, `1.2` for the
> `has_approval_block` predicate) behind the `ClassifierProvider` seam, scoring against a versioned YAML rule
...
Numbering is **proposed, never forced** — Mara can accept the existing codes wholesale (common when an org already has a scheme) or apply the configured scheme to normalize. A human correction must be non-blank when the decision is recorded; the checklist also blocks blank identifiers persisted by an older deployment before review closes, and the commit write boundary retains the same guard as defense in depth. All identifiers are validated unique at commit.

### 8.3 Owner & process-link proposals
- **Owner** is proposed from embedded author / a folder-owner mapping / the process's `OrgRole` (Clause 5.3 data), defaulting to *unassigned* if low confidence — Mara assigns on review (UJ-2 step 6). The review UI selects from the user directory, shows a stable ID suffix to disambiguate duplicate display names, and sends the full stable user ID; commit also resolves legacy exact directory identity strings (including UUID-shaped subjects) inside the run's org. The pre-commit checklist blocks an unresolved/ambiguous human assignment while the decision can still be corrected. Before review closes, commit resolves the assignment again under a directory-row lock and records the stable ID in the run's internal `commit_owner_snapshot` — not in the human decision history — so a later disable/retire cannot alter the reviewed choice. Partial resume performs the same preflight for remaining items; an unresolvable legacy label stays partial and can receive a narrowly scoped correction, while a stable ID can be recovered after disable/retire. The worker retains tenant/non-guest/existence validation as a write-boundary backstop.
...
- **Conflicts** (duplicate or legacy-blank identifiers, unresolved/ambiguous reviewed owners, unresolved ambiguous items above a configurable count) — must be resolved or explicitly deferred. This revalidation also catches the old bulk-picker placeholder `Quality Manager` and pre-validation blank decisions before they can leave editable review.
...
- Resume validates only failed/remaining materialization inputs (not already-committed identifiers, which now collide with the vault by design). A legacy unresolved owner or blank identifier returns a repairable blocker while the run stays `PartiallyCommitted`; a per-file `CORRECT` decision is accepted only for a failed/unattempted item, never one already committed.
...
|---|---|---|
| `import_run` | id, org_id, source_root, status, created_by, committed_by, classifier_version, ocr_enabled, profile, counts(jsonb), commit_owner_snapshot(jsonb), created_at, completed_at | the run object + internal per-file reviewed-owner materialization snapshot |
