...
| **Reviewed ownership** | The folded reviewed owner is materialized on both imported Documents and Records. Ownership is separate from commit attribution: `created_by` / `captured_by` and the `import_baseline` signer remain the human committer. An unresolved best-effort engine hint falls back to the committer; an unresolved or ambiguous explicit human assignment fails the item. |
