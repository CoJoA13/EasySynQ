...
- **Conflicts** (duplicate identifiers, unresolved/ambiguous reviewed owners, unresolved ambiguous items above a configurable count) — must be resolved or explicitly deferred. This revalidation also catches the old bulk-picker placeholder `Quality Manager` on in-flight runs before they can enter a non-reviewable `PartiallyCommitted` state.
...
| **Reviewed ownership** | The folded reviewed owner is materialized on both imported Documents and Records. Ownership is separate from commit attribution: `created_by` / `captured_by` and the `import_baseline` signer remain the human committer. An unresolved best-effort engine hint falls back to the committer. An unresolved or ambiguous explicit human assignment blocks the initial commit while the run remains reviewable; the review→commit transaction snapshots the validated directory ID so a subsequent account lifecycle change does not alter that accepted assignment, and the worker still fails closed on a missing or cross-org snapshot. |
