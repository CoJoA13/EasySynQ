...
- **Owner** is proposed from embedded author / a folder-owner mapping / the process's `OrgRole` (Clause 5.3 data), defaulting to *unassigned* if low confidence — Mara assigns on review (UJ-2 step 6). The review UI selects from the user directory and sends the stable user ID; commit also resolves legacy exact directory identity strings (including UUID-shaped subjects) inside the run's org. The pre-commit checklist blocks an unresolved/ambiguous human assignment while the decision can still be corrected; the commit worker retains the same fail-closed check as a write-boundary backstop.
...
| **Reviewed ownership** | The folded reviewed owner is materialized on both imported Documents and Records. Ownership is separate from commit attribution: `created_by` / `captured_by` and the `import_baseline` signer remain the human committer. An unresolved best-effort engine hint falls back to the committer. An unresolved or ambiguous explicit human assignment blocks the initial commit while the run remains reviewable; the worker also fails closed if one reaches the write boundary. |
