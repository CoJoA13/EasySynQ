...
> **Escalation dependencies (reconciled per Decisions Register R29; as-built in slice S-notify-6).** "Assignee's manager" resolves against the nullable **`manager_id`** reporting-line FK on **`app_user`**; when that is unset the escalation **falls back to the QM / OrgRole** (e.g. `org_role: "Quality Manager"`). The `timer_sweep` evaluates the **before/after offsets** — the reminder (`task.due_soon`) and escalation (`task.escalated`) thresholds — as **business-day** offsets against the **`working_calendar`** entity (org holidays / working days), so a reminder gives real working-day lead time and an escalation never pings a manager over a weekend/holiday. **Overdue (`task.overdue`) has no offset** — it fires at `due_at` itself; snapping `due_at` to a working day is a *materialize-time* concern (`engine._due_at` is currently raw wall-clock — a separate, still-deferred working-calendar reconcile, NOT this timer-offset slice).
