...
as_of: "2026-09-09"
baseline_commit: "929eb6e9e34501c68ea534d5264bcf477e968021"
last_shipped_slice: "S-audit-checkpoint-v2-codec"
migration_head: "0092"
next_migration: "0093"
api_unit_tests: 2510
web_test_files: 281
...
The focused codec suite passed **143 tests**, and the three affected legacy unit modules passed
**173 tests**, without skips. Five independently generated byte/signature/hash vectors and 31 public
key-admissibility inputs are frozen as synthetic fixtures. The complete local API suite passed
**2,510 tests with one release-only skip in 88.66 seconds**. The existing actual-image test was
explicitly enabled: its immutable production image ran all five vectors and 31 key controls offline
as UID 10001, with development packages absent. PyCryptodome 3.23.0 is now a direct runtime dependency;
all 132 locked package versions and artifacts stayed unchanged.

The separate current-image external verifier acceptance passed all **three mandatory runtime cases**,
preserving the existing live/historical custody and denial checks. Both disposable acceptance images
and the external runner's private directory were removed. Ruff, formatting across **792 API files**,
mypy across **454 source files**, runtime-runner static checks, **95 authority fixtures**, repository
authority, site-data across all **2,000 tracked and new candidate files**, and whitespace checks passed.
Source, producer, test outputs, image identities and the real Git index were verified. This local
verification does not replace required CI on the published candidate commit.

Other full-stack counts are attributed to
[MR !22](https://gitlab.com/synqsuite-group/EasySynQ/-/merge_requests/22)'s GitLab pipeline
`2832210477` on source `04245add28b1b176413a17ae7e5e4ef0339385e5`, whose merged tree is source base
`929eb6e`. All fourteen required checks passed: 1,259 integration passes with two existing skips,
285 response contracts, 2,352 web tests and 80 browser tests. Post-merge pipeline `2832249805` also
passed all fourteen checks on that base. These full-stack results are inherited; the new local
results above cover this codec candidate. No live enrollment, grant, key rotation, restore,
