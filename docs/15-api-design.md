...
| GET | `/risks/register` | (any member) | — | **(S-risk-1b)** The head lifecycle status `{ exists, register_doc_id, identifier, state, current_effective_version_id, has_governing }` (org-level, not row-sensitive). **(S-context-fe)** Also carries the optional, server-computed `can_release`/`can_manage` capability booleans (GET-only) — the faithful steward gate (see release row). |
...
| POST | `/risks/register/release` | `document.release` | — | **(S-risk-1b)** T6 (Approved→Effective). Enforced over the **multi-axis** `_register_release_scope` (artifact_id + folder_path + document_level + lifecycle_state + SoD-2; author/approver ≠ releaser) — **NOT SYSTEM** (the S-risk-5 release posture). **(S-context-fe)** The FE `RegisterLifecyclePanel` now gates on the server-computed `can_release` on `GET /risks/register` (the faithful multi-axis answer — it dropped the single-axis ARTIFACT probe, which couldn't fold folder/level/SoD-2). The shared INV-1 SERIALIZABLE cutover. |
...
| GET | `/context/register` | (any member) | — | The head lifecycle status `{ exists, register_doc_id, identifier, state, current_effective_version_id, has_governing }`. **(S-context-fe)** Also carries the optional, server-computed `can_release`/`can_manage` capability booleans (GET-only) — the steward console's faithful gate (the shared `register_caps` helper, byte-identical to `/risks/register`). |
...
| POST | `/context/register/release` | `document.release` | — | T6 (Approved→Effective). Enforced over the **multi-axis** `_register_release_scope` (artifact_id + folder_path + document_level + lifecycle_state + SoD-2; author/approver ≠ releaser) — **NOT SYSTEM** (the S-risk-5 release posture). **(S-context-fe)** The Context SPA's `RegisterLifecyclePanel` gates Release on the server-computed `can_release` on `GET /context/register` (the faithful multi-axis answer — a single-axis SYSTEM/ARTIFACT FE probe can't replicate this scope). Held by no seeded role → a SYSTEM override in v1. The shared INV-1 SERIALIZABLE cutover. |

### 8.10d Interested Parties register (`/interested-parties`)

> **As-built (S-interested-parties-1, R51):** the clause-4.2 Interested Parties register — the **third register family**, a near-byte-identical clone of the §8.10c Context register. Clause 4.2 is **ORG-LEVEL** — the `interested_party` satellite carries `org_id` (the doc-14 §6 correction) but **no `process_id`**. Gates ride the already-seeded `register.read` / `register.manage` / `document.release` at the **SYSTEM** scope — **NO new key, catalog stays 102, NO new role grant** (the QMS Owner is the steward; a bound Process-Owner's PROCESS `register.manage` grant matches no party row). `GET /interested-parties` is a **filter-not-403** read (a no-grant caller gets `200`+empty; all-or-nothing at SYSTEM); the single-row GET enforces `register.read` @ SYSTEM. The IPR head is reserved from every generic document path — the create / mutate / DCR-implement / import quad (`_MANAGED_REGISTERS = {RSK, CTX, IPR}` + the inline ingestion-import guard). Content model = enriched + influence (`party_type` spine + `party_name` + `needs_expectations` + optional `influence` + `status` + `last_reviewed_at`); no graded axis (only the rows freeze). **The `GET /interested-parties/summary` governing read + the Management-Review 9.3.2(b) consumer shipped in `S-interested-parties-2`** (the summary is `register.read` @ SYSTEM, 403-on-deny; the MR `CONTEXT_CHANGES` input sources BOTH the 4.1 Context AND 4.2 Interested-Parties governing summaries as a nested `{context, interested_parties}` envelope — un-gapping the long-carried `CONTEXT_CHANGES` gap; NO migration, NO new key, NO new `ReviewInputType` enum member). **The SPA is deferred to `S-interested-parties-fe`.**

| Method | Path | Perm | Idem | Notes |
|---|---|---|---|---|
| GET | `/interested-parties` | `register.read` @ SYSTEM | — | Filter-not-403 (org-level → all-or-nothing); the SPA filters/sorts client-side. |
| POST | `/interested-parties` | `register.manage` @ SYSTEM | — | `{ party_type, party_name, needs_expectations, influence?, last_reviewed_at? }`; a new party is always `status=active`. `409` unless the head is Draft/UnderRevision; lazily mints the `IPR` head on the first row. |
| GET | `/interested-parties/summary` | `register.read` @ SYSTEM | — | **(S-interested-parties-2)** The GOVERNING (Effective) register summary `{ published, total, by_party_type, by_influence (+`unspecified` for NULL), by_status, active, never_reviewed }` — the read-of-record (never the live satellite). 403-on-deny (org-level). Mounted **before** `/{id}` (the str-convertor shadow). |
| GET | `/interested-parties/{id}` | `register.read` @ SYSTEM | — | 403-on-deny. |
| PATCH | `/interested-parties/{id}` | `register.manage` @ SYSTEM | — | Partial; an explicit null clears `influence`/`last_reviewed_at`; a null on party_type/party_name/needs_expectations/status → 422. `409` unless the head is editable. |
| GET | `/interested-parties/register` | (any member) | — | The head lifecycle status `{ exists, register_doc_id, identifier, state, current_effective_version_id, has_governing }`. Also carries the optional, server-computed `can_release`/`can_manage` capability booleans (GET-only) — the steward console's faithful gate (the shared `register_caps` helper, byte-identical to `/context/register` + `/risks/register`). |
| POST | `/interested-parties/register/start-revision` | `register.manage` @ SYSTEM | — | T7 (Effective→UnderRevision). `409` unless Effective. |
| POST | `/interested-parties/register/publish` | `register.manage` @ SYSTEM | — | Freeze the working rows into a new version and submit for review. `409` unless Draft/UnderRevision; rejects a zero-row register. |
| POST | `/interested-parties/register/release` | `document.release` | — | T6 (Approved→Effective). Enforced over the **multi-axis** `_register_release_scope` (artifact_id + folder_path + document_level + lifecycle_state + SoD-2; author/approver ≠ releaser) — **NOT SYSTEM** (the S-context/S-risk-5 release posture); the server-computed `can_release` is the faithful FE gate. Held by no seeded role → a SYSTEM override in v1. The shared INV-1 SERIALIZABLE cutover. |
...
> **S-ing-4b — now UI-backed (web, 2026-06-08; naming reconciled 2026-07-28).** The full `/admin/imports/*` surface above is surfaced by the **Import Review UI** at the canonical SPA routes `/imports` and `/imports/{runId}`. The former `/ingestion` routes remain redirect-only bookmark compatibility and preserve the run id and query string. The implementation package remains `apps/web/src/features/ingestion/` because ingestion is the pipeline activity, while **Import** is the user-facing resource name. Front-end only — **no endpoint, database, or permission-contract change**. Note for FE consumers: `run.counts` is a **flat, top-level-merged bag** (`by_band`, top-level `quarantine`, `dedup`, `proposal`, `commit` — no `queues`/`review` block), and the folded review stats (`undecided`/`kind_confirmed`/`keep_items`) are read from `…/checklist`.
