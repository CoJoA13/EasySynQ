...
> slice, PR #41) shipped the initial CLI paths:** source-dependent restore-to-scratch verification
> (checkpoint-not-ahead + chain re-verify, but no cutover), upgrade orchestration (pre-backup → migrate
> → health-gate; full production safety remains open), and archive v2. The archive always carries the
> PG dump + blob-locator manifest; encryption is conditional, and realm/config/checkpoint legs are
> best-effort. S11 also shipped Caddy strict-static-CSP/TLS-1.2-floor + a server-side NFR P95 smoke +
...
- **Backup/restore (R37).** `easysynq backup` writes a timestamped, checksummed archive whose
  mandatory contents are `pg_dump` + a MinIO locator/hash manifest. Encryption is conditional;
  realm, config, and checkpoint legs are best-effort and may be absent. `easysynq restore` verifies
  into a scratch database and a unique prefix in the configured shared scratch bucket while reading
  blob bytes from the configured source store. It checks checkpoint ordering and re-verifies the
  restored chain, but it neither cuts over nor proves recovery after source-store loss. A
  self-contained generation, PITR↔blob-snapshot alignment, role-preserving cutover, and derived-index
  rebuild procedure remain open. Gate G-C is a separate transient-plaintext integrity drill.
...
| D-6 | Mirror drift posture; PITR scope | — | **MVP = R11 RO-mount + nightly regeneration/overwrite (satisfies AC#2); drift *detection* (scan/quarantine/alarm) is v1.** **MVP backup = nightly dump + source-dependent scratch verification. Self-contained recovery, cutover, and continuous WAL/PITR remain open.** |
...
- [x] NFR P95 budgets (server-side smoke, S11); migrations reversible + CI-gated; air-gapped bundle installs offline (third-party digest-pin gated on a `v*` tag release run); security headers + TLS 1.2+ (Caddy default floor, `caddy validate`-confirmed); secrets rotatable. **WCAG 2.2 AA = deferred with the v1 web track** (the MVP ships the backend + the setup-wizard/admin web only; the full clause-aligned SPA + its axe/manual a11y pass is v1).
