...
- **Conventions.** `Idempotency-Key` on mutating POSTs (Redis dedupe); `ETag`/`If-Match` optimistic lock (412); cursor pagination over UUID v7; **RFC 9457 problem+json** with canonical `code`s (`permission_denied`/`illegal_transition`/`lock_conflict`/`etag_mismatch`/`worm_required`/`two_tier_violation`/`validation_error`/`not_found`/`rate_limited`/`sod_violation`/`setup_incomplete`); **429 + Retry-After** Redis token bucket (per-user + per-IP) at app, coarse at Caddy.
...
| **S5 Approval + SoD** | Approve/release persist append-only `signature_event`; SoD blocks self-approval | `signature_event` written in same txn; append-only (rescind via `voided_*`); only v1 meanings emitted; `content_digest`+`auth_context` captured, Part-11 cols NULL; **[PROOF]** **SoD-1** (author of the version cannot approve it) → **409**; **SoD-2** (author never releases own edit; approver-release behind `allow_approver_release` flag) and **SoD-3** (auditor independence — InternalAuditor role hard-excludes edit/approve/release) covered, evaluated against **immutable version/audit history** (INV-4), not a single current field. |
...
| `signature_event` subjects `record_id`/`capa_stage_id` | S5/S19 | columns present; targets created empty (S21) |
