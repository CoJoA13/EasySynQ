...
ADR 0005 deliberately introduces a staged bootstrap identity claim because Keycloak and PostgreSQL cannot commit atomically. The claim, Keycloak recovery marker, provider revalidation before definitive release, shared administrator-set serialization, and active credential-receipt digest add state-machine and maintenance cost, but they prevent duplicate administrator grants, stale claim release, destructive compensation, and acknowledgment of a superseded temporary password after partial or concurrent failures. Remove this machinery if identity provisioning, credential delivery, and application persistence gain one transactionally attested boundary; the provider-call lock-duration cost is tracked separately in `bootstrap-provider-lock-duration`.
