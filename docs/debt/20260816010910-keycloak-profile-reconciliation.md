...
Keycloak 26.7 exposes both realm user-profile policy and individual user updates as whole representations without an ETag or compare-and-swap token. EasySynQ must reconcile only the required flags for email, first name, and last name plus the exact admin-only managed `easysynqBootstrapClaim` definition at realm level, and a pending marker-bound bootstrap retry must reconcile only that user's email/name fields while preserving every other returned field and attribute. The managed definition is required because Keycloak's default disabled unmanaged-attribute policy silently ignores an unknown marker; EasySynQ preserves that policy instead of broadly enabling unmanaged attributes. Application writers are serialized and both paths write only when reconciliation is needed, but a simultaneous external administrator edit between GET and PUT could still be overwritten; replace or fence these boundaries when versioned mutation becomes available or external profile administration becomes supported.
