...
6. **Setup and admin actions are audited**: every setup step and every admin action writes append-only audit. The current backup gate verifies archive and object integrity against the configured source store; it is not a source-independent recovery guarantee. Identities are retired, never erased, preserving attribution for ISO traceability and future Part-11 e-signatures.
