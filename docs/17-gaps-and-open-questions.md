...
- **Residual P1/P2 issues still needing a fix (listed at the very end):** **B4 (P1)** doc `11` §5.8 wizard wireframe; **B12 (P3→affects UI)** doc `11` §5.4 lock TTL; **B11 (P3)** doc `14` `task` "My Actions" label; **B5 (P2)** doc `15` §8.7 DCR-is-a-Record line. None are integrity/security blockers; all are one-line/one-wireframe back-propagation misses, but B4 was an original **P1** and is genuinely not done in `11`.
...
| **B4** (wizard step count/order) | **PARTIAL (R4)** | `08` §3 IS the canonical 10-step flow (Step 0 Bootstrap … Step 10, blocking restore-test gate G-C before auth, org-profile before storage) — that half is done and correct. **BUT `11` §5.8 was NOT updated:** its wireframe still shows the OLD **8-step** wizard (`Welcome & License → Admin → Org/realm → Storage → Backup → Identity → Import → Review`) with **no Step 0 Bootstrap, no restore-test gate, license-first ordering** — exactly the divergence the finding flagged. `11` §5.8 must be re-drawn to match `08` §3. **Residual P1.** |
| **B5** (DCR Record vs workflow) | **PARTIAL (R22)** | `05` §5/§5.5, `10` §3.1, and `14` §7 (`dcr` entity + `dcr_stage_event`) correctly model the DCR as a mutable-state **workflow object, NOT `kind=RECORD`**, lifecycle Open→…→Closed (+Cancelled/Rejected). **BUT `15` §8.7 still contradicts this:** it says "A `dcr` is itself **a Record, immutable once closed** (`14 §1.2`)" — the pre-R22 framing. The `15` §8.7 sentence must be corrected to "a controlled workflow object with a mutable `state`, not a `kind=RECORD` artifact." **Residual P2.** |
...
| **B11** ("My Tasks" vs "My Actions") | **PARTIAL (R23)** | "My Tasks" is canonical and used in `01` glossary, `10` §8 (retires "My Actions" explicitly), `11`, `13`. **BUT `14` §7 still labels the `task` entity "The atom of **My Actions** (`10 §8`)"** — one stray legacy label in the data model. Also `15` §5.2/§8.8 use "My-Actions" as an endpoint/inbox name (`/me/actions`); arguably acceptable as an API path but the prose label should read "My Tasks" for consistency. Fix the `14` `task`-row label. **Residual P3.** |
| **B12** (lock TTL 8h vs 24h) | **PARTIAL (R24)** | `04` §5.2 and `05` §4.2 both correctly say **8h** (canonical). **BUT `11` §5.4 was NOT updated:** the Check-out wireframe still reads "Lock holder: Priya · auto-expires in **24h** (extendable)" — the exact stale value the finding flagged. `11` §5.4 must change 24h → 8h. **Residual P3.** |
...
| **D8** (task-inbox name + search shortcut) | **PARTIAL (R23)** | Search shortcut fully RESOLVED (Cmd-K/Ctrl-K primary + `/` secondary in `11` §2.1/§2.4 and `13` §2.1). Task-inbox name is PARTIAL for the same reason as B11 — one stray "My Actions" label in `14` §7's `task` row. |
...
- **The meta-risk in §E is *mostly* closed — but doc `11` is the exception.** The original §E warned that `14` was authoritative but the source sections were never edited. That is now **largely** untrue: `01`, `03`, `04`, `05`, `06`, `07`, `08`, `09`, `10`, `12`, `13`, `14`, `15`, `16` carry explicit "reconciled per Decisions Register Rn" annotations with canonical tokens reproduced verbatim. **However the UI/UX doc `11` was essentially NOT touched by the reconciliation pass** — it still carries the OLD 8-step wizard (§5.8), the OLD 24h lock TTL (§5.4), and the 5-state document-state palette is only annotated upstream, not in `11`. A build team reading `11` in isolation will implement the pre-reconciliation wizard and lock TTL. This is the single biggest residual.
...
- **Leakage scan was clean for the worst tokens.** No `record.retire`, no uppercase `signature_event.meaning`, no `audit_qms.*` (outside explicit normalization notes), no `import.initiate`/`import.administer` (outside the "these replace…" note in `09`). The two stray legacy labels that DID survive are narrow: `14` §7 task row says "My Actions"; `15` §8.7 says the DCR "is a Record."

### Residual P1/P2 issues that STILL need fixing

1. **(P1) `11` §5.8 — First-Run wizard wireframe is the old 8-step flow.** Re-draw to the canonical `08` §3 ten-step flow: add **Step 0 Bootstrap**, the **blocking backup + restore-test gate (G-C)** before authentication, order **org profile before storage**, and drop the license-first ordering. (Finding B4 / R4.)
2. **(P2) `15` §8.7 — DCR mislabeled as "a Record, immutable once closed".** Correct to: a controlled **workflow object** with a mutable `state` column (NOT a `kind=RECORD` artifact); only its closed form is retained as a record-like snapshot. (Finding B5 / R22.)
3. **(P3) `11` §5.4 — lock TTL still "24h".** Change to **8h** to match `04` §5.2 / `05` §4.2. (Finding B12 / R24.)
4. **(P3) `14` §7 — `task` row still labeled "The atom of My Actions".** Change to "My Tasks". (Findings B11 / D8 / R23.)

None of the four are integrity, security, authorization, audit-chain, or data-model-correctness defects — the load-bearing resolutions (R1, R2, R3, R5, R6, R7, R8, R9, R10, R11, R12, R13, R14, R18, R20, R21, R25, R27, R35, R37) are all fully and verbatim back-propagated. The residuals are back-propagation misses concentrated in the UI doc (`11`) plus two stray sentences (`14`, `15`).
