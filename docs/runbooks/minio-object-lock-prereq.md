...
The controlled vault's integrity rests on **object-lock**: the `documents` bucket must be created
**object-lock-enabled at creation time** — it **cannot be retro-added** (R37). Setup gate **G-B**
verifies this (the WORM probe) and refuses to finalize otherwise.

## Provision the vault bucket (object-lock + GOVERNANCE)
The Compose `minio-init` provisions this for the dev stack. For a production/external MinIO/S3:
```bash
...
## Restore implication
**Never restore into the locked `documents` bucket.** `easysynq restore` always targets the plain
non-WORM `restore-scratch` bucket (and the drill does too). Cutover lands the verified blobs into a
**fresh** object-lock-enabled bucket — see [backup-restore.md](backup-restore.md). The off-host
