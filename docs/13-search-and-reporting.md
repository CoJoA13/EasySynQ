...
> **As-built (S-risk-2 / S-interested-parties-2):** the **9.3.2(e)** `RISK_OPPORTUNITY_ACTIONS` input now sources the clause-6.1 Risk & Opportunity register's GOVERNING (Effective) frozen snapshot, and the **9.3.2(b)** `CONTEXT_CHANGES` input is **no longer a sourceless gap** — it sources BOTH the clause-4.1 `ContextRegister` AND the clause-4.2 `InterestedParty` register governing summaries, projected as a nested `{context, interested_parties}` envelope (each half `null` when its register is unpublished; `available` if either is published, a gap only when both are). Both ride the review owner's `register.read` (fail-closed → a gap row) and read the CONTROLLED governing snapshot (never the live working satellite), frozen into the WORM minutes. The four remaining sourceless 9.3.2 inputs are c1 (customer satisfaction) / c7 (supplier performance) / d (resource adequacy) / f (improvement opportunities).
