...
| T5 | **Approved → InReview** | Ken / Mara (QM) | `document.rescind_approval` | Not yet Effective | Void prior approval signature (recorded, not deleted); re-open review; **audit: APPROVAL_RESCINDED**. |
| T6 | **Approved → Effective** | Mara (QM) / Ken; or system (Beat) at effective date | `document.release` | Effective date ≤ now (or release-now override perm `document.release_now`); WORM blob confirmed; rendition present | **Atomic supersession** (§3.4): set this version Effective, set prior Effective → Superseded, update `document.effective_version_id`, set `effective_from`/prior `effective_to`; enqueue mirror-sync + distribution + acknowledgement tasks; write **`signature_event`** (meaning=`release`); **audit: RELEASED + SUPERSEDED(prior)**. |
| T7 | **Effective → UnderRevision** | Priya / Diego (Owner) | `document.checkout` + `document.revise` | No active Working Draft already; lock acquirable | Acquire lock; create new Working Draft seeded from Effective source blob; `document.current_state = UnderRevision`; **prior Effective keeps governing & stays in mirror**; **audit: REVISION_STARTED**. |
| T8 | **UnderRevision → Effective** (abandon) | Priya / Diego | `document.discard_draft` | Active draft exists | Discard Working Draft (scratch removed; nothing entered the version chain); release lock; revert `current_state` to Effective; **audit: REVISION_ABANDONED**. |
...
- **Break-lock requires a confirm warning.** A holder of `document.force_checkin` (typically Mara, QM) is shown an explicit warning — who currently holds the lock, since when, and that their in-progress work will be preserved as recoverable scratch — and must confirm before the lock is broken. The action is audited (`LOCK_BROKEN`) and the current lock-holder is notified.
...
`document.create`, `document.read`, `document.read_draft`, `document.checkout`, `document.checkin`, `document.revise`, `document.discard_draft`, `document.force_checkin`, `document.submit`, `document.review`, `document.approve`, `document.rescind_approval`, `document.release`, `document.release_now`, `document.obsolete`, `document.edit_metadata`, `document.manage_distribution`, `document.export`, `document.print`.

> **Permission-key normalization (reconciled per Decisions Register R5).** These keys are the doc 07 canonical catalog forms. Earlier variant spellings used in this section normalize as: `document.view` → `document.read`; `document.view_drafts` → `document.read_draft`; `document.read_effective` → `document.read`; `document.submit_for_review` → `document.submit`; `document.make_obsolete` → `document.obsolete`; `document.export_controlled` → `document.export`. `document.checkin` stays `document.checkin` (or folds into `document.edit` per doc 07). The record-disposition permission elsewhere is `record.dispose` (**NOT** `record.retire`).
