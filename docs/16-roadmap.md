...
| **Authority + integrity checks before data** | First-run setup, the permission engine, and the source-dependent archive/scratch gate must work before any real QMS content lands. Source-independent recovery remains an open release blocker (doc 08). |
...
| **Self-hosted operability is a feature** | One-command Compose install, sizing profiles, and a source-dependent archive/scratch integrity drill are part of "done," not afterthoughts; source-independent recovery is a separate release gate. |
...
| **Backup/restore** | `easysynq backup`/`restore`, nightly default, and the source-dependent archive/scratch integrity drill in the runbook. Source-independent recovery remains open per R7. | 03 §9 |
...
- Setup completes only after the **source-dependent archive/scratch integrity drill** passes; this is
  not source-independent recovery proof.
...
| **R7** | **A source-store loss is not recoverable from the current non-self-contained archive**, so scratch verification is not recovery proof. | Med / High | Gate G-C packs, checksums, unpacks, and restores a transient plaintext tar into scratch, then verifies locators/object hashes against the configured source store. It does not decrypt or test a retained durable archive. Self-contained object-byte generation plus a role-preserving cutover proof remain open and release-blocking; the documented integrity drill does not close them. Only PG + MinIO are backup-critical (everything else is rebuildable), which limits but does not eliminate the gap. |
...
EasySynQ is built **foundation-first**: the MVP proves the hardest implemented guarantees — a controlled vault, an enforced single-Effective lifecycle with immutable versions and recorded approvals, a deny-by-default granular permission engine, a guided first-run with source-dependent archive/scratch integrity verification, and a tamper-evident audit trail. Source-independent object recovery and role-preserving cutover remain open release blockers. With the implemented foundation proven, **v1** completes a full ISO 9001:2015 QMS: records and evidence, audits and CAPA, drift detection, search, notifications, Evidence Packs, and scaled import — making all seven user journeys real and the success metrics measurable. **v1.x** turns a working product into a confidently operable one (reporting, bulk ops, delegation/SoD UX, observability, scale, locales, and a whole-vault portable export for tenant migration/decommission — R33). **Future** activates the locked-decision extensions — Part 11 e-signatures, multi-standard frameworks, AI-assisted classification, and optional HA — each **additive on a reserved hook, never a rewrite**, because every prior phase was built to leave the door open while keeping today's product calm, focused, and self-hosted.
