...
| **R7** | **A source-store loss is not recoverable from the current non-self-contained archive**, so scratch verification is not recovery proof. | Med / High | The current gate decrypts and restores the database into scratch and verifies locators/object hashes against the configured source store. Self-contained object-byte generation plus a role-preserving cutover proof remain open and release-blocking; the documented integrity drill does not close them. Only PG + MinIO are backup-critical (everything else is rebuildable), which limits but does not eliminate the gap. |
