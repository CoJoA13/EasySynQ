...
> **Implementation note (slice S-rec-4).** The capture snapshot pins the policy **id** + `retention_basis_date`, but the sweep **live-dereferences** the pinned policy's `duration`/`disposition_action`/`review_required`. So a policy *edit* would reach already-captured records — which is what the spec *wants* for an **extension** ("an extension can be applied forward") but must not do for a **reduction**. The asymmetry is enforced two ways: a `PATCH /retention-policies/{id}` is **extend-forward only while a policy has non-disposed pinned records** (a reduction is refused `422 retention_reduction_blocked`), and **shortening retention for future captures is done by archiving the policy and creating a shorter one** (soft-archive hides a policy from new-capture resolution; records already pinned keep being swept under it). No per-record duration snapshot is needed. PATCH omission means “leave unchanged”; explicit `null` may clear only nullable `applies_to` and `worm_lock_period`, while required policy fields reject it with `422 validation_error`.
...
- **Segregation of duties — creator ≠ disposer (SoD-6, slice S-rec-4, doc 07 §7).** The principal who captured a Record (`record.captured_by`) may **not** execute its disposition to `DISPOSED`/`DESTROY` — the attempt is refused `409 sod_self_disposition` and audited `DISPOSITION_REFUSED_SOD` — unless the org sets `allow_self_disposition = true` (the small-org config flag, default OFF). The gate is enforced in-service (not the PDP), so a SYSTEM `record.dispose` override does not bypass it; it gates only the DISPOSED edge (never the DUE_FOR_REVIEW / ACTIVE re-anchor), is exempt for the Beat sweep (a system actor, no human capturer), and is subsumed by the stronger dual-control (requester ≠ approver) on the R27 WORM-destroy-under-legal-order hatch.

> **Implementation note (slice S-rec-2 + Issues #359/#360).** `retention_until = retention_basis_date + duration`; a `PERMANENT` duration (paired with `disposition_action=RETAIN_PERMANENT`) or an unset (`event:*`) basis date yields no expiry and is **never auto-swept**. The DESTROY path is **purge-after-commit, reaper-backstopped** (Batch-5 review 2026-07-22): the record flips to `DISPOSED`, its evidence `blob` rows (and any structured-PDF rendition row) are deleted, and a `pending_blob_purge` marker is written — all **committed first** — and only then are the WORM bytes physically removed as a separate idempotent step. Every new marker is bound in that same transaction to the exact Record and immutable DESTROY `disposition_event`; an R27 marker also names the executed two-person `worm_destroy_request`. The hourly `reap_pending_blob_purges` worker revalidates those rows and derives bypass authority from them — it never treats the marker's boolean as authority. It accepts only the configured records/renditions buckets and checks live ownership by the physical `(bucket, object_key)`, independent of the diagnostic/untrusted marker SHA, so a false SHA cannot hide a recaptured/live object and a valid event cannot authorize deletion from an unrelated S3 namespace. Capture, immediate purge, and reaper recovery also take the same transaction-scoped advisory lock for that exact physical identity before promotion or the live-owner check. A purge that wins the lock removes the old bytes before capture promotes its staged copy; a capture that wins commits its owner before purge re-checks and drops the stale marker. Thus the ownership check and S3 erase are atomic with respect to byte-identical recapture. Multi-evidence capture resolves PostgreSQL's actual `hashtext` keys, de-duplicates them, and sorts those numeric lock keys before acquisition; raw object-key order is not collision-safe. Immediate purge and each reaper iteration claim the marker row before the object lock; the reaper re-claims later batch snapshots after every per-marker commit/rollback releases the original batch claims. Invalid bound work is warned and dropped without touching storage; pre-0081 unbound work is legacy and can replay only without bypass. A storage failure *after* the disposition commit leaves the record legally `DISPOSED` with durable work for that reaper; **a storage error never rolls back a disposition** (a client that retries a deferred-but-committed disposition receives `already_disposed`). The ordering was inverted from the original purge-first design specifically to preserve the backup invariant — the `blob` row is deleted **at the commit**, so a backup (which iterates `blob` rows) never references the transiently-orphaned bytes. This admits a brief, safe, reaper-recoverable **"tombstone before the bytes are physically gone"** window, but never a `blob` row over missing bytes (nor missing bytes under a live `blob` row). `place_legal_hold`/`release_legal_hold` require a `reason` (audit trail); a release returns the record to `ACTIVE` (the next sweep re-evaluates expiry). The sweep emits `RECORD_DISPOSITION_DUE` as the v1 **"notify the owning `org_role`" surrogate** until the doc-10 notification engine lands. A `DISPOSED` record's `content_hash` survives as the tombstone — the immutable audit trail (not a re-hash of absent bytes) is the post-disposal verification.
...
- Crash recovery preserves that same boundary: governance bypass is replayed only when the marker's executed R27 request and immutable disposition event agree on the Record, organization, two distinct actors, and legal basis. A forged or mismatched marker is not a substitute for dual control.
- **Derivative copies are part of the same legal erasure.** If a sealed Evidence Pack copied the
  Record's bytes or embedded its metadata in a Finding/CAPA dossier, the R27 transaction makes that
  pack terminal `UNAVAILABLE`, revokes its shares, disposes its registered pack Record through a
...
sequenceDiagram
    ...
    API->>PG: persist current generator + accepted request IP with BUILDING
    API->>Worker: enqueue pack-build job
    Worker->>PG: load attempt context + evaluate current read grants
...
- **Scope-limited by construction, but never *silently* so.** The pack only ever contains items the current build generator is permitted to see; generation runs through the same deny-by-default RBAC+ABAC engine (Architecture §8.3). The generate transaction records that generator and the accepted request IP, and the worker evaluates current grants under that persisted context (R58). When an over-broad scope causes items to be left out, the generator (e.g., Mara) is **warned prominently** during the preview/generate flow — never left to assume the pack is complete — and the pack records **which items were excluded and why** (permission vs. genuine absence) in the exclusion report (§7.3 item 7), distinct from the compliance gap report. A pack that drops evidence without surfacing it is treated as a defect (reconciled per Decisions Register R28).
...
- **Delivery to external auditors.** Olsen receives a **time-boxed, read-only guest link** (Vision & Scope: Time-boxed Guest Access) or a downloaded archive — never standing access to the live vault. The pack is a frozen snapshot under ordinary retention; revoking a link does not alter its content. The explicit exception is an R27 legal order over copied content: every affected pack becomes terminal `UNAVAILABLE`, its live links are revoked, and its retained ZIP/portfolio bytes are erased (§5.5 / R27).
...
>
> **S-pack-2** (migration `0026`) completes UJ-7 with external delivery + the PDF portfolio. **Delivery** uses an Ed25519 **signed-token-outside-the-PEP** path (reusing the S7c verify-token key, **domain-separated** by a `easysynq.packshare.v1` preamble + a distinct 105-byte token length so a verify token can never cross-validate as a share token; mint **fails closed** if the key isn't durably persisted), with a DB-backed **`pack_share_link`** record (`token_digest` only — the raw token is returned once, never stored; `recipient`/`expires_at`/`revoked_at`+`by`+`reason`/`download_count`; state derived from the nullable timestamps). `POST /evidence-packs/{id}/share` (gate `report.evidence_pack.generate` — the pack-management authority, catalog CLOSED) mints a time-boxed link; `…/share-links` lists them; `…/share-links/{id}/revoke` is immediate. The **public, no-auth, latch-exempt** guest surface (`GET /evidence-packs/shared?t=` landing + `…/shared/download?t=&format=zip|pdf`) re-checks the **revocable** DB row on every access (revoke takes effect at once), audits `PACK_DOWNLOADED`, and **streams** the bytes through the API (no presigned URL outlives a revoke; `Referrer-Policy: no-referrer`; the raw token is never logged). The landing surfaces the gap + exclusion summary (R28 honesty). The **PDF portfolio** (`format=pdf`) is built at seal **Stage 2** (a separate transaction after the seal commits, best-effort so a Gotenberg outage never blocks the canonical pack): a cover + traceability index + gap/exclusion pages + each version's **cached** §11.3-stamped rendition (a truthful pure-pypdf "no longer governs" overlay if its state changed; an honest placeholder when uncached — so the **API never invokes Gotenberg**), content-addressed into the non-WORM renditions bucket; the per-request guest stamp reuses `watermark.stamp_per_request_copy` (the S7d export precedent). **Deferred (v1.x / later):** the heavier `guest_grant` table + the `scope.evidence_pack_id` ABAC predicate + a Keycloak guest identity (the token path supersedes them for v1); guest-link `ip_allow` binding; app-layer rate-limiting (a Caddy edge concern).
>
> **S-aud-capa-pack** (migration `0039`) completes the §7.1 **Finding/CAPA scope** (the Audits/CAPA family close-out). `PackScopeKind += FINDING|CAPA` (additive `ALTER TYPE`; no new permission keys — pack create/download ride `report.evidence_pack.generate` / `report.export`). A FINDING/CAPA pack resolves **only the records linked AS EVIDENCE** to the finding(s) / the CAPA's stages (`evidence_for_link(target_type=finding|capa_stage)`) — the closure-verification evidence files (Implement-completion + Verify-effectiveness) come through here; the finding/CAPA **subject is never a pack_item** (a record subtype has no `evidence_blob` → no verifiable ZIP bytes). The subject's substance is a synthesized, content-hash-**sealed dossier** (§7.3 items 2/5 realized for this scope): `findings/<id>.json` / `capas/<id>.json` carrying the finding's fields + correction chain + linked CAPA, or the CAPA's full append-only **stage trail** (RootCause→ActionPlan→Verify, grouped by `cycle_marker`) with each stage's **e-signature metadata** (signer + meaning + content_digest + time) + per-stage evidence + the origin finding — so an auditor can "prove this NC was closed effectively" from the ZIP alone. **PII boundary:** signers/creators are projected to `{user_id, display_name}` only (never `email`/`keycloak_subject` — the ZIP is externally shareable). **Seal:** the dossier folds into `pack_content_hash` as **v2** (`easysynq.evidencepack.v2`; CLAUSE/PROCESS stay byte-identical **v1**); `dossier_digest` hashes the sorted per-file SHA-256s so it is reconstructable from `manifest["dossier"]["files"]` (self-verifying per §7.4). **Gap report = N/A** for finding/CAPA scope (no clause-coverage concept; the cover/manifest/PDF render N/A, never a misleading 0-of-0). The PDF portfolio's verify-scheme line tracks the v2 seal.
>
...
> `FAILED`.
>
> **Issue #363 / R58 / migration `0083`** makes the locked pack row authoritative for the
> asynchronous build principal. `generate` writes `build_requested_by` and the accepted request's
> `build_source_ip` atomically with `BUILDING`; Celery still receives only `pack_id`. The worker
...
> A missing/non-active initiating user also fails closed; no worker path falls back to `created_by`.
