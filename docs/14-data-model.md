...
| `workflow_definition` | `id` PK, `org_id`, `key` (`document_approval`), `version` int, `effective` bool, `subject_type` enum(`DOCUMENT`,`DCR`,`CAPA`,`AUDIT`,`MGMT_REVIEW`,`PERIODIC_REVIEW`,`DOC_ACK` *(S-ack-1)*), `stages` jsonb, `entry_conditions` jsonb, `default_sla` jsonb | Declarative, versioned, data-not-code (`10 §2`). |
