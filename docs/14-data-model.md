...
| Entity | Key attributes | Notes / source |
|---|---|---|
| `app_user` | `id` PK, `org_id`, `username` (unique/org), `email`, `display_name`, `keycloak_sub`, `status` enum(`INVITED`,`ACTIVE`,`LOCKED`,`DISABLED`,`RETIRED`), `mfa_enrolled`, `session_invalidated_at`, `is_guest` bool, `valid_until` (guests), `manager_id` uuid FK → `app_user.id` null, `color_scheme` enum(`LIGHT`,`DARK`,`AUTO`) NOT NULL | `08 §15.2`, `07`, `12 §2.5`. **Never hard-deleted** (attribution). PII anonymizable on `RETIRED` while `id` stable (`12 §9.4`). `manager_id` is the reporting-line FK that notification escalations resolve against (falling back to the QM/OrgRole where unset) (reconciled per Decisions Register R29). **S-railfoot-pref (mig `0092`)** adds `color_scheme`, the account-level interface colour-scheme preference (Decisions Register R69); `AUTO` means follow the operating system and is the backfilled value for every pre-existing account. |
...
| `structured_pdf_blob_sha256` (**S-rec-3**) | text null, **NO FK** | Pointer to the cached structured-record PDF — a DERIVED, regenerable rendition (§5.4) in the non-WORM renditions bucket, built best-effort at Stage 2. Capture enqueues it immediately; an hourly bounded missing-pointer redrive retries dropped/failed builds and recognizes legacy optional-form rows from their pinned `field_schema`. The pinned schema is the discriminator, so ad-hoc JSON such as a `KPI_READING` grading snapshot is never rendered as a structured form. Keyset paging advances the bounded publish batch beyond malformed legacy pins, so an invalid oldest page cannot starve valid records. The selector and row-locked builder exclude a `DESTROY` disposition event, so delayed work cannot recreate a purged rendition; `ARCHIVE_COLD`/`TRANSFER` may still finish one because they preserve content. Plain Text (the `evidence_pack.zip_blob_sha256` R27 precedent) so the WORM-destroy hatch never aborts; nulled + the `blob` row dropped on destroy (blob-row-iff-bytes). NOT part of `content_hash`. |
...
| Entity | Key attributes | Notes |
|---|---|---|
| `evidence_pack` | `id` PK, `org_id`, `framework_id`, `title`, `scope_kind` enum(`CLAUSE`,`PROCESS`,`FINDING`,`CAPA`), `scope_selector` jsonb (`clause_ids`/`process_ids`/`finding_ids`/`capa_ids`), `embedded_record_ids_at_seal` jsonb null (exact dossier-only shared-PK Record ids; `[]` for new non-dossier seals, null only for legacy seals), `period_start/end` (date overlay), `status` enum(`DRAFT`,`BUILDING`,`SEALED`,`FAILED`,`UNAVAILABLE`), `build_started_at`, `build_requested_by` FK→`app_user` null (**RESTRICT**), `build_source_ip` Text null (lossless accepted representation for exact-string `ip_allow` replay), `item_count`, `gap_summary` jsonb, `exclusion_summary` jsonb, `content_hash` (the domain-separated manifest seal), `zip_blob_sha256` (**plain Text, NO FK to `blob`**), `portfolio_blob_sha256` (**S-pack-2** — the cached PDF portfolio pointer; plain Text, NO FK; nullable, a derived rendition), `pack_record_id` FK→`record` (nullable until sealed), `invalidated_at` null, `invalidated_by_disposition_event_id` FK→`disposition_event` null, `error`, `created_by`, `created_at`, `generated_at` | The pack header. Sealed = an immutable, content-addressed ZIP written to the WORM `records` bucket + registered as a `RETAIN_PERMANENT` EVIDENCE `record` (`pack_record_id`). The ZIP blob is reached via `pack_record_id → evidence_blob → blob` — `zip_blob_sha256` carries **no `blob` FK** so the pack's R27 WORM-destroy hatch never aborts on a RESTRICT FK (`06 §7.4`). Issue #361 adds terminal `UNAVAILABLE`: an R27 source event is mandatory, both artifact pointers are null, shares are revoked, and the preserved header/membership/seal becomes the audit tombstone. The embedded-id snapshot makes R27 discovery match immutable dossier bytes instead of later mutable correction pointers; a legacy null uses a seal-time-bounded compatibility lookup. Invalidation closes transitively over each affected pack's registered Record and exact artifact aliases, so packs may safely contain earlier packs. Downgrade from `0082` refuses while derived purge work or immutable `PACK_INVALIDATED` events exist; an otherwise compatible `UNAVAILABLE` tombstone maps to pre-0082 terminal `SEALED` with its pointers still null. Issue #363 / R58 makes the current attempt fields authoritative across Celery redelivery: `generate` overwrites both with its caller/IP in the `BUILDING` transaction, the task carries only `pack_id`, and the worker uses current grants/time plus the replayed IP. `created_by` remains the DRAFT-header creator and is never an implicit worker fallback. |
...
`event_type` gains `PACK_GENERATED` / `PACK_BUILD_FAILED` (S-pack-1), `PACK_SHARED` / `PACK_DOWNLOADED` / `PACK_SHARE_REVOKED` (S-pack-2), and `PACK_INVALIDATED` (Issue #361 R27 derivative erasure); `audit_object_type` gains `evidence_pack` (all pack lifecycle + delivery events key on the pack-header id). **S-pack-2 took the Ed25519 signed-token-outside-the-PEP delivery path** (the `pack_share_link` row above); the heavier `guest_grant` table + the `scope.predicates.evidence_pack_id` ABAC bound (§3, above) + a Keycloak guest identity stay **deferred to v1.x** (the token path supersedes them for v1).
...
Every exported `manifest.json` record entry snapshots `content_hash_version` beside
`content_hash`; Finding/CAPA dossier subject files and the `dossier_subjects` index do likewise.
This is export metadata, not a new table field, and makes a standalone pack containing both v1 and
v2 record seals unambiguous to re-verifiers.
...
erDiagram
    ...
    RECORD ||--o{ PENDING_BLOB_PURGE : "authority-bound recovery work"
    DISPOSITION_EVENT ||--o{ PENDING_BLOB_PURGE : "authorizes"
    DISPOSITION_EVENT |o--o{ DISPOSITION_EVENT : "one-hop pack-copy lineage"
    DISPOSITION_EVENT |o--o{ EVIDENCE_PACK : "R27 invalidates"
    WORM_DESTROY_REQUEST |o--o{ PENDING_BLOB_PURGE : "optionally authorizes R27 bypass"
...
| Entity | Key attributes | Notes / source |
|---|---|---|
| `retention_policy` | `id` PK, `org_id`, `name`, `applies_to` jsonb (record_type/clause/process), `basis` enum(…), `duration` (ISO-8601 / `PERMANENT`), `disposition_action` enum(`DESTROY`,`ARCHIVE_COLD`,`TRANSFER`,`RETAIN_PERMANENT`), `review_required` bool, `worm_lock_period` (≥ duration), **`active` bool (S-rec-4 soft-archive; default true), `archived_at`, `archived_by` FK, `created_at`, `updated_at`** | Policy-as-data. The capture snapshot pins only `policy_id` + `retention_basis_date` (`06 §5.1`); `duration`/`disposition_action`/`review_required` are **live-dereferenced** at sweep time, so a policy *edit* propagates to pinned records — the one-way ratchet (`06 §5.2`) is honored by the **extend-forward PATCH guard** (a reduction is refused while records are pinned) + **soft-archive** (`active=false` stops new-capture auto-attach; pinned records keep being swept; shorten-for-future = archive + create a shorter policy). The seeded System Default is protected from archive/rename but remains org-configurable. `Sealed Evidence Pack Retention` is a separate system-managed, immutable `PERMANENT` + `RETAIN_PERMANENT` policy with a stable per-org id; migration `0077` re-pins existing packs and the build path lazily ensures it for future orgs. A pre-0077 same-name user policy is renamed in place without changing its settings, id, or references. CRUD via `/retention-policies` (`15 §8.16`, `retention.read`/`retention.manage`, R38). |
| `disposition_event` | `id` PK, `org_id`, `record_id` FK, `action`, `tombstone` bool, `policy_id` FK **null** (null for a non-policy legal-order destroy), `approved_by` FK **null** (null on a system Beat-sweep auto-dispose), `requested_by` FK **null** (the R27 first authorizer), `is_worm_destroy` bool, `legal_basis` text null, `derived_from_disposition_event_id` self-FK null, `executed_at` | Immutable; the executed-disposition tombstone — blob removed/anonymized but Record metadata + audit preserved (`06 §5.3`). Slice S-rec-2 added the root R27 dual-control fields. Issue #361 adds exactly one derivative hop for a disposed pack Record: it copies the actors/legal basis and points to the source event instead of fabricating a second destroy request. |
...
| `pending_blob_purge` | `id` PK, `org_id` FK, diagnostic `sha256`, physical `bucket` + `object_key`, requested `bypass_governance`, `record_id` FK null, `disposition_event_id` FK null, `worm_destroy_request_id` FK null, `authority_bound` bool, `created_at` | Durable purge-after-commit recovery work (`06 §5.3`). Every new row is server-defaulted `authority_bound=true` and must identify its Record and immutable DESTROY event; a requested bypass also requires the executed R27 request. The reaper derives authority from those rows, accepts only the configured records/renditions buckets, and uses `(bucket, object_key)`—not the marker SHA—for live-object ownership. A derivative pack marker additionally requires its event to be the one-hop child of the request's lawful source event and its target Record to be the registered Record of that source-invalidated `UNAVAILABLE` pack. Existing rows upgraded by `0081` are marked legacy (`authority_bound=false`) and may recover only without bypass. The app role has column-scoped INSERT and only `UPDATE(id)` for `FOR UPDATE SKIP LOCKED`; it cannot choose legacy mode or mutate the target/authority fields. |

**Constraints:** resolution precedence per-record > process > clause > type > system default; `legal_hold = true` overrides expiry; DESTROY blocked until `worm_lock_period` expired AND no hold (a blocked destroy is logged-as-refused-with-reason, `RECORD_ERASURE_REFUSED`, R27); `RETAIN_PERMANENT` pairs with `duration=PERMANENT` (never auto-swept); reduction of retention never applies to already-captured records (extension forward only). The R27 dual-control destroy is the **only** pre-lock-expiry destruction path (two distinct authorizers, `BypassGovernanceRetention`, GOVERNANCE-only), and its reaper replay requires the same executed request plus its matching immutable disposition event (R27 Issue-360 addendum). Under the Issue-361 addendum, that authority extends only to exact sealed derivative copies: build stages hold an organization-scoped shared transaction lock, R27 holds the exclusive side, and each unavailable pack's cleared pointers + source event are enforced as one database shape.
