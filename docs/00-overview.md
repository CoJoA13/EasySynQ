...
| **What does it enforce?** | At most one **Effective** version per document (DB-enforced; an active released document has one), immutable content/frozen metadata, mandatory change reasons, recorded approvals, version-pinned records, retention + controlled disposition, and a full audit trail. |
...
| **How is it run?** | One Linux host via Docker Compose. S and M sizing overlays ship; L remains a reserved design target. Data never leaves the org boundary; admin-controlled backups; no telemetry without explicit opt-in. |
| **What is the tech?** | React + TypeScript + Mantine + Tailwind (SPA); Python 3.12 + FastAPI (API); PostgreSQL 18 + MinIO + Redis; Celery workers; Keycloak (auth); Gotenberg/LibreOffice (rendering); Caddy (TLS). Shipped search is PostgreSQL FTS behind an OpenSearch-ready seam. |
...
| **D4 — Tech stack** | **A modern, recommended self-hostable stack.** | React/TS + Mantine + Tailwind; FastAPI/Python; PostgreSQL + MinIO + Redis; Celery; Keycloak; Gotenberg; Caddy; Docker Compose. PostgreSQL FTS is current; OpenSearch is a reserved extension. OpenAPI-first, 12-factor, deny-by-default. | doc 03 (full rationale vs. alternatives) |
...
> **Reconciliation note.** The `decisions-register.md` is the **single authoritative source of truth and reconciliation source** for this specification: it records the locked foundational decisions, the locked stakeholder decisions, and all registered normative resolutions. **Where it conflicts with any text in sections 01–15, the register supersedes that text**, and the drift items in §8 below are now back-propagated and superseded by it (by R-number) rather than merely flagged. The numbered files run **00–18**: the fifteen substantive sections (01–15), the roadmap (16), the **gap audit** (17 — `17-gaps-and-open-questions.md`, the *input* the register resolves, not a normative section), and the **build plan** (18 — `18-mvp-implementation-plan.md`, the slice plan + §1 canon corrections). The TOC below is authoritative for what exists. A future appendix/standards-pack section is anticipated by the multi-standard roadmap (doc 16, Future) but is not yet written.

| # | File | One-line description | Reads after |
|---|---|---|---|
| **★** | `decisions-register.md` | **Authoritative reconciliation source & single source of truth** — locked foundational decisions, locked stakeholder decisions, and all registered normative resolutions. **Supersedes any conflicting text in 01–15**; canonical tokens/enums/states/field-names are verbatim. | 00 (consult continuously) |
...
| **03** | `03-architecture-and-stack.md` | System architecture & technology stack: shipped S/M packaging plus reserved L/OpenSearch/observability targets, C4 diagrams, security posture, backup/restore, and binding non-functional requirements. | 01, 02 |
...
| **08** | `08-setup-and-onboarding.md` | Admin first-run design and onboarding model. The shipped browser wizard has six blocking screens; deferrable user/role/process/import work continues after finalize in the named application surfaces. | 03, 07, 09 |
...
| **13** | `13-search-and-reporting.md` | Search & reporting design: current PostgreSQL-FTS metadata search and Compliance Checklist, plus clearly labeled future content search, OpenSearch facets, dashboards, and audit-log search. | 02, 06, 12 |
...
**Working / operational docs** (not part of the numbered spec, but kept current): `current-status.md` (dated execution snapshot) · `open-residuals.md` (sole current residual ledger) · `manuals/` (installation, user, and administrator/IT manuals) · `decisions-register.md` (★ above) · `slice-history.md` (historical shipped-slice changelog/evidence) · `dev-workflow.md` (operator/`.env` + per-feature API quick-reference) · `runbooks/` (operator runbooks) · `superpowers/{specs,plans}/` (per-slice design specs + plans — see `superpowers/README.md` for the index; read the one matching your slice, not eagerly). Reusable engineering traps live in `.claude/rules/engineering-patterns.md` as a retained compatibility asset.

> **Build status — what's shipped vs designed.** Sections 01–18 are the **static design spec** (the *intended* system), not a progress board. For what is actually **built**, read `current-status.md`; use `slice-history.md` for dated shipped evidence and `open-residuals.md` for current deferred work.
...
2. **Document control prevents drift by inverting authority, and revision management is the discipline on top of it.** The Controlled Vault (PostgreSQL + MinIO) holds the master; the filesystem mirror is regenerated read-only from **Effective versions only**, so the disk can never silently become a competing truth (D2). Revision management (doc 05) adds the version model — immutable snapshots, **at most one Effective version** (DB-enforced partial-unique index; an active released document has one), check-out/check-in under a Redis lock with a mandatory change reason, and the DCR that records *why* a change happened. Together these structurally close the three classic drift vectors (the wall poster, the intranet PDF, the laptop Word file). What can still drift *outside* the boundary — a tampered mirror file, a stale printout — is **detected** (mirror re-hash + auto-correct, controlled-rendition verify token, scheduled re-review).
...
2. **Then choose your path by role:**
   ...
   - *Any reader resolving an apparent conflict:* go straight to the `decisions-register.md` (all registered decisions + the Part-4 finding→resolution map) — it is the authoritative reconciliation source.
