...
| **What is the tech?** | React + TypeScript + Mantine + Tailwind (SPA); Python 3.12 + FastAPI (API); PostgreSQL 16 + MinIO + Redis; Celery workers; Keycloak (auth); Gotenberg/LibreOffice (rendering); Caddy (TLS). Shipped search is PostgreSQL FTS behind an OpenSearch-ready seam. |
