...
The specification is **17 numbered documents** plus the **Decisions Register**: this overview (00), fifteen substantive sections (01-15), and the roadmap (16), all governed by the `decisions-register.md`. The order below is the recommended reading order; the "Reads after" column gives the hard prerequisites.

> **Reconciliation note.** The `decisions-register.md` is the **single authoritative source of truth and reconciliation source** for this specification: it records the locked foundational decisions, the locked stakeholder decisions, and the normative resolutions (R1–R37) to every gap-audit finding. **Where it conflicts with any text in sections 01–15, the register supersedes that text**, and the drift items in §8 below are now back-propagated and superseded by it (by R-number) rather than merely flagged. The authoring brief referred to sections "01..17". There is **no separate numbered file 17** in the canon (the gap audit `17-gaps-and-open-questions.md` is the input the register resolves); "17" is the *total numbered-document count* (00 through 16 inclusive). The TOC below is authoritative for what exists. A slot for a future appendix/standards-pack section (a natural "17") is anticipated by the multi-standard roadmap (doc 16, Future) but is not yet written.

| # | File | One-line description | Reads after |
|---|---|---|---|
| **★** | `decisions-register.md` | **Authoritative reconciliation source & single source of truth** — locked foundational decisions, locked stakeholder decisions, and the normative resolutions R1–R37 to every gap-audit finding. **Supersedes any conflicting text in 01–15**; canonical tokens/enums/states/field-names are verbatim. | 00 (consult continuously) |
...
2. **Then choose your path by role:**
   ...
   - *Any reader resolving an apparent conflict:* go straight to the `decisions-register.md` (R1–R37 + the Part-4 finding→resolution map) — it is the authoritative reconciliation source.
...
| C6 | TOC count "01..17" in the brief vs. files that exist (00-16). | There is no numbered file 17 in the canon. | **No numbered file 17 exists.** "17" = total numbered-document count (00-16); the gap audit `17-gaps-and-open-questions.md` is the audit input the Decisions Register resolves, not a spec section. A future standards-pack/appendix section is anticipated by doc 16 (Future) but unwritten. |
