...
>
> **Migration pre-flight**: 0075 now counts rows with `octet_length(scope_ref) > 2000` before creating the index and **refuses with an actionable message** if any exist — because such a row could predate the cap and would otherwise abort the upgrade with a raw PostgreSQL error. It deliberately does **not** rewrite them: `audit_event` is append-only and hash-chained, so correcting a legitimate audit record is an operator decision under a signed-off correction, never something a migration does silently. Proven end-to-end on PG16 — with an over-long row injected the upgrade refuses and the index is not created; once removed it succeeds.
