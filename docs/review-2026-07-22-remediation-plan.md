...
- [x] `services/packs/build.py:304` — pack FINDING/CAPA subjects are serialized with NO subject read-check (`record.read` gates only the evidence candidates) → R28 bypass; ADD a per-subject `capa.read`/`finding.read` gate at the subject's own scope (not via the evidence classifier) and refuse/exclude when unreadable `[f]` (fixed in two stages: Batch 6 added the shared refuse-any read graph at create/generate; **Issue [#363](https://github.com/CoJoA13/EasySynQ/issues/363) / R58** persists the current build initiator + accepted source IP, replays only that IP with current grants/time, and repeats the same audited graph in the worker immediately before dossier serialization. Candidate R28 classification, sealed-Record capture, and lifecycle events now use the current attempt generator, never the DRAFT creator.)
- [x] `services/packs/service.py:620` — public pack share survives a WORM destroy (cached portfolio PDF keeps serving); disposition must invalidate share tokens + purge derived artifacts + fail-closed on disposition state `[f]` (fixed in two stages: Batch 6 added the conservative serve-time `pack_has_destroyed_member` guard; **Issue [#361](https://github.com/CoJoA13/EasySynQ/issues/361)** adopts the R27-only full-erasure decision — terminal `UNAVAILABLE`, source-event lineage, live-share revocation, pack-Record disposition, ZIP/portfolio purge-after-commit + authority-bound reaper recovery, and build-vs-erasure serialization. Ordinary retention keeps the independent pack.)
...
- [x] `infra/compose/compose.yml` — Keycloak `start-dev` with container-local H2 (fixed: an optimized
  ...
  A fail-safe one-time offline H2 exporter runs before standard installer/`just up`/appliance-helper
  recreation, preserves users, stable IDs, credential hashes and client edits, validates its staged
