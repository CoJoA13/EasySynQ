...
| `S-backup-objects` | C-01 | XL | An object-bytes leg (`services/backup/objects.py` + a `backup objects` CLI verb + Beat task, enumerated under the same REPEATABLE READ snapshot the dump uses), and a cutover-shaped restore: today objects land flat at `{restore_id}/{sha}` while restored `blob` rows keep their original `(bucket, object_key)` — and reads resolve the *stored literal* (`api/documents.py:1974`), so the runbook's "repoint MinIO at a fresh bucket" cannot produce a resolvable namespace. Also: restoring an older archive after an R27 erasure now fails the triad, so archives silently expire. **Blocked on D-B.** |
