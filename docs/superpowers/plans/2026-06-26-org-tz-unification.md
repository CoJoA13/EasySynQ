...
- **In that Linux development environment `pytest -m unit` has a known 17-failure baseline** (ProactorEventLoop / symlink) — run **targeted** unit files (`cd apps/api && uv run pytest tests/unit/test_<x>.py`) for a clean local signal; treat the full suite as CI-authoritative.
