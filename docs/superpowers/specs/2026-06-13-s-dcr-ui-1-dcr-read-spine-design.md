...
6. **F6 — gating/smoke.** Per-key calm-403; `demo` (System Administrator) holds **no `changeRequest.*`** content keys → SYSTEM overrides on the **live** demo `app_user` row (org ORG_EXAMPLE) for the smoke, granted **before login** so the nav appears on first load.
...
- **Resulting version (F4):** when `target_document_id` is set (**REVISE** → the revised doc; **RETIRE** → the now-Obsolete target), a read-only deep-link "View document" → `/documents/{target_document_id}` (the documents page self-gates; note there is **no `?version=` deep-link** — the link lands on the document page, not the specific version). ⚠ **CREATE edge — CONFIRMED deferred:** a CREATE DCR has `target_document_id` null and `resulting_version_id` on a **new** document whose id `_dcr` does **not** expose; the verification sweep confirmed **no top-level `GET /versions/{id}`** and every version-read endpoint is nested under `/documents/{document_id}/` (rejecting a mismatched doc id) — so the SPA **cannot** resolve a bare `version_id` → `document_id`. For CREATE, show the resulting-version id as **text, no link** (named, not faked; a future `resulting_document_id` enrichment or a version-lookup endpoint unblocks it). Link absent entirely when `resulting_version_id` is null (pre-implement / RETIRE-without-cutover).
...
> **Resolved (was a plan investigation) — user-id display.** `created_by` and stage-event `actor_id` are bare `app_user.id`s. The codebase-wide precedent is the shared **`useUserDirectory()`** roster (`app/shell/useUserDirectory.ts` → `GET /directory/users` → `DirectoryUser[] {id, display_name}`), resolved inline via `directory.find(u => u.id === id)?.display_name ?? \`${id.slice(0,8)}…\`` (exactly `CapaTimeline.actorLabel`). The DCR drawer calls `useUserDirectory()` and passes `directory` down; `actor_id === null` → "system". No backend enrichment, no per-row `useUser` hook.
...
2. **Grant SYSTEM overrides on the LIVE `demo` `app_user` row** (org **ORG_EXAMPLE**), **before login** so the nav appears first load: `changeRequest.read` (the page) + `document.read` (full target resolution; omit it to demo the calm-degrade). Check the override lands on the row matching the **live login's Keycloak subject** (the re-created-JIT-row trap — verify against the live `app_user`, not the old bootstrap row).
