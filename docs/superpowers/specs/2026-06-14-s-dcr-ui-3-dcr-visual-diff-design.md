...
2. **Grant `document.read_draft` + `changeRequest.read`** to the LIVE login's `app_user` row (org **ORG_EXAMPLE**), via the JIT-safe "assign to ALL org-ORG_EXAMPLE users" pattern (`scripts/grant-overrides.py`) — the live `demo` login JIT-maps to a *specific* `app_user` row.
