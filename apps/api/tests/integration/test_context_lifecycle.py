"""S-context-1 integration proofs — the Context register's controlled-document
publish/freeze/release lifecycle (clause 4.1).

A register goes Draft → (publish) InReview → (approve) Approved → (release) Effective: the working
rows are FROZEN into an immutable version at publish (clause 4.1 has no scoring criteria — rows
...
⚠ The CTX head is a per-org SINGLETON shared across the one-org integration DB, so a lifecycle test
that drives it to Effective would 409 every other test's context-create. The head-advancing test
therefore NORMALIZES the head to editable at the start (advancing a prior crashed cycle if needed)
and
RESTORES it to editable at the end — self-provisioning, non-polluting (the risk-lifecycle
discipline).
...
from easysynq_api.db.models._context_enums import ContextClassification
...
from easysynq_api.services.context import add_context_issue
...
def subj() -> SimpleNamespace:
    ...
    return SimpleNamespace(
        steward=f"kc-clf-s-{salt}",
        approver=f"kc-clf-a-{salt}",
        releaser=f"kc-clf-r-{salt}",
    )


@pytest.fixture
async def restore_context_head(
    app_client: AsyncClient, token_factory: Callable[..., str], subj: SimpleNamespace
) -> Any:
    """Teardown guard for the head-advancing lifecycle test: restore the shared per-org CTX head to
    editable so sibling tests' context-creates succeed — even if the body fails mid-lifecycle (the
    risk restore_register_head precedent; a 500 after release() commits would otherwise leave it
...
async def _create_issue(
    client: AsyncClient, h: dict[str, str], *, description: str = "C"
) -> dict[str, Any]:
    r = await client.post(
        "/api/v1/context",
        headers=h,
        json={"classification": "internal", "description": description},
    )
...
async def _status(client: AsyncClient, h: dict[str, str]) -> dict[str, Any]:
    r = await client.get("/api/v1/context/register", headers=h)
...
async def _approve_and_release(
    ...
    rel = await client.post("/api/v1/context/register/release", headers=hrl)
...
async def _drive_to_editable(
    ...
    """Normalize the org's shared CTX head to an EDITABLE state (Draft/UnderRevision or absent),
    ...
    for _ in range(6):
        ...
        if state == "Effective":
            assert (
                await client.post("/api/v1/context/register/start-revision", headers=hs)
            ).status_code == 200
            return
        if state == "InReview":
        ...
        if state == "Approved":
            assert (
                await client.post("/api/v1/context/register/release", headers=hrl)
            ).status_code == 200
            continue
...
async def test_register_publish_freeze_and_revision_lifecycle(
    ...
    snapshot carrying the edited row. The restore_context_head teardown returns the head to editable
    ...
    row = await _create_issue(app_client, hs, description="aging workforce")
    head_id = row["register_doc_id"]

    pub = await app_client.post("/api/v1/context/register/publish", headers=hs)
    assert pub.status_code == 200, pub.text
    assert pub.json()["state"] == "InReview"

    # the frozen Draft version carries the rows under the context_register key (no criteria block).
    async with get_sessionmaker()() as s:
        ...
        reg = (v.metadata_snapshot or {}).get("context_register")
        assert reg is not None
        assert any(r["id"] == row["id"] for r in reg["rows"])
        assert "criteria" not in reg  # clause 4.1 has no scoring criteria (unlike risk)
    ...
    rel = await app_client.post("/api/v1/context/register/release", headers=hrl)
    assert rel.status_code == 200, rel.text
    released = rel.json()
    ...
    add = await app_client.post(
        "/api/v1/context", headers=hs, json={"classification": "external", "description": "late"}
    )
    assert add.status_code == 409, add.text
    patch = await app_client.patch(
        f"/api/v1/context/{row['id']}", headers=hs, json={"status": "closed"}
    )
    assert patch.status_code == 409, patch.text

    # --- the revision: edit → publish → approve → release SUPERSEDES in place ---
    sr = await app_client.post("/api/v1/context/register/start-revision", headers=hs)
    assert sr.status_code == 200, sr.text
    assert sr.json()["state"] == "UnderRevision"
    edited = await app_client.patch(
        f"/api/v1/context/{row['id']}",
        headers=hs,
        json={"status": "closed", "category": "weakness"},
    )
    ...
    async with get_sessionmaker()() as s:
        ...
        reg2 = (v2.metadata_snapshot or {}).get("context_register")
        ...
        assert frozen_row["category"] == "weakness"
...
async def test_row_write_serializes_on_head_lock(
    ...
    """add_context_issue locks the CTX head FOR UPDATE, exactly as publish_register does while it
    ...
    add_context_issue BLOCKS until it releases, then completes (without the lock it would commit
    immediately, failing the .done() assertion). The S-records-W held-lock-blocks idiom (the
    test_risk_lifecycle precedent). Adds rows only → the head stays editable (non-polluting)."""
    author = f"ctx-ser-{uuid.uuid4().hex[:8]}"
    await _grant(author, "register.manage")
    ha = _auth(token_factory, author)
    first = await _create_issue(app_client, ha)  # mint/find the head + a first row (head editable)
    ...
    worker = sm()
    try:
        # Hold the head FOR UPDATE, exactly as publish_register / add_context_issue do.
        ...
        assert not add_task.done(), "add_context_issue did not block on the held head lock"
