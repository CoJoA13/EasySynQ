...
from easysynq_api.db.models._vault_enums import DocumentCurrentState, DocumentKind, DocumentLevel
...
def _document_type(*, code: str = "SOP") -> DocumentType:
    """A detached catalog row; the canonical builder reads only its stable code and level."""
    return DocumentType(
        id=uuid.uuid4(),
        org_id=uuid.uuid4(),
        code=code,
        name="Procedure",
        document_level=DocumentLevel.L2_PROCEDURE,
    )
...
def test_framework_scoped_deny_wins_over_broad_system_allow() -> None:
    ...
    resource = resource_from_doc(
        _doc(framework_id=fw), document_type=_document_type(), process_ids=frozenset()
    )
    grants = [
...
def test_concrete_type_scoped_deny_wins_over_broad_system_allow() -> None:
    """A concrete-type DOC_CLASS DENY matches the stable catalog code.

    Blanking only ``concrete_type`` recreates the pre-#345 shape and flips DENY to ALLOW, proving
    this test distinguishes completion of the dormant selector.
    """
    resource = resource_from_doc(
        _doc(framework_id=uuid.uuid4()),
        document_type=_document_type(code="SOP"),
        process_ids=frozenset(),
    )
    grants = [
        _grant(Effect.ALLOW, ScopeLevel.SYSTEM, {}),
        _grant(
            Effect.DENY,
            ScopeLevel.DOC_CLASS,
            {"document_level": "L2_PROCEDURE", "concrete_type": "SOP"},
        ),
    ]
    assert authorize(grants, "document.read", resource, _CTX).allow is False

    old_shape = dataclasses.replace(resource, concrete_type=None)
    assert authorize(grants, "document.read", old_shape, _CTX).allow is True
