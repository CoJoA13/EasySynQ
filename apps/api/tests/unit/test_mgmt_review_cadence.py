...
from easysynq_api.services.management_review.cadence import (
    MR_REVIEW_LEAD_DAYS,
    management_review_state,
    next_mr_due,
)
...
def test_mr_review_state_buckets() -> None:
    ...
    assert management_review_state(None, datetime.date(2026, 6, 1)) is None  # not scheduled
    assert management_review_state(due, datetime.date(2026, 9, 1)) == "overdue"  # today == due
    assert management_review_state(due, datetime.date(2026, 9, 2)) == "overdue"  # past due
    lead = due - datetime.timedelta(days=MR_REVIEW_LEAD_DAYS)
    assert management_review_state(due, lead) == "due_soon"  # exactly on the lead boundary
    assert (
        management_review_state(due, lead - datetime.timedelta(days=1)) == "current"
    )  # before the lead
