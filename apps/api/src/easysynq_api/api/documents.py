...
async def create_document_endpoint(
    ...
    # ALSO carries the kind + framework_id the row will land with (create_document assigns
    # kind=DocumentKind.DOCUMENT + the org iso9001 framework), or a kind/FRAMEWORK-scoped
    # document.create DENY is silently dropped at creation (deny-always-wins / R3) — the pre-create
    # sibling of the merged resource_from_doc completion (#333/#346), covering every READ/decision
    # builder but not this create builder (no row to read yet). Both are exactly what the row
    # carries the instant after create, so a scope DENY evaluates identically pre/post-create.
