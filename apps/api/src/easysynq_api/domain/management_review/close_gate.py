...
parallel). The tail is ``task_state is not TaskState.DONE`` so a ``None`` task-state (an ACTION
whose task was never spawned/linked) BLOCKS **fail-closed** rather than silently passing — mirroring
...
``spawned_task_id`` so an unlinked ACTION yields ``None``) and filters with this single predicate,
so the rule has one source of truth (unit-tested alone)."""
