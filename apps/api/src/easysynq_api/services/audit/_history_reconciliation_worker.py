"""Fresh reconciliation executable; OS ceilings precede application imports/input."""
...
def _main() -> int:
    ...
    try:
        ...
        send({"op": "READY", "version": protocol.VERSION})
        sequence = 0
        while True:
            ...
            try:
                if store is None:
                    ...
                    ceiling = scope.limits.maximum_spool_bytes
                ...
                elif op == "PACKAGE_BEGIN":
                    wire.fields(request, {"op", "id", "kind", "index", "length"})
                    length = wire.integer(request["length"], 0, scope.limits.maximum_total_bytes)
                    store.package_begin(request["kind"], request["index"], length)
                    offset = 0
                    while offset < length:
                        body = wire.decode_chunk(
                            _read_frame(stdin), sequence, offset, length - offset
                        )
                        store.package_chunk(body)
                        offset += len(body)
                    end = wire.metadata(_read_frame(stdin))
                    wire.fields(end, {"op", "id"})
                    if end["op"] != "PACKAGE_END" or wire.integer(end["id"]) != sequence:
                        wire.invalid()
                    store.package_end()
                elif op in {
                    "RESERVE_PAGE",
                    ...
                    if op == "RESERVE_PAGE":
                        ...
                        response["ticket"] = (
                            dataclasses.asdict(ticket) if ticket is not None else None
                        )
                    elif op == "PAGE_BEGIN":
                    ...
                    wire.fields(request, {"op", "id", "expected_observations"})
