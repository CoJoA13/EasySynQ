...
async def user_system_domain_keys(
    session: AsyncSession, user_id: uuid.UUID, org_id: uuid.UUID
) -> set[str]:
    """The system-domain permission keys ``user_id`` effectively holds — role-derived AND
    override — what the two-tier credential-reset guard checks before a content-tier caller may
    reset their Keycloak password (R35). Mirrors ``granted_permission_keys``' query shape, each
    leg additionally joined against ``Permission.is_system_domain.is_(True)``."""
    role_keys = (
        (
            await session.execute(
                select(Permission.key)
                ...
                .where(
                    RoleAssignment.user_id == user_id,
                    RoleAssignment.org_id == org_id,
                    Permission.is_system_domain.is_(True),
                )
            )
        )
        .scalars()
