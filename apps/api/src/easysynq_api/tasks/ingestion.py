"""Celery tasks for the ingestion pipeline (slices S-ing-1/2, doc 09 §3-6).

The pipeline auto-chains via ``.delay``-triggered tasks (NOT Beat-scheduled): ``scan_source`` →
``extract_source`` → ``classify_source`` (each enqueues the next after its commit). All are
idempotent under ``task_acks_late`` re-delivery (status-guarded + per-(run,file) upsert) and
fail-closed (the packs build/reaper discipline). ``reap_stalled_runs`` is the Beat job that recovers
a run wedged in any in-progress stage (a hard worker kill strands it) → FAILED, freeing the
source-root lock. Each uses a fresh disposed async engine per ``asyncio.run`` on the app DSN (the
non-owner ``easysynq_app`` role — the import_* tables are granted to it in 0029/0030)."""
...
from ..services.ingestion import reap_stalled_runs, run_classify, run_extract, run_scan
from .app import app
...
def classify_source(run_id: str) -> None:
    """Score the four classification dimensions, then rest at Classified (idempotent)."""
