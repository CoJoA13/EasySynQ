...
it("names the subject (identifier + title) when the enriched list provides it", async () => {
  server.use(
    http.get("/api/v1/tasks", () =>
      HttpResponse.json([
        {
          ...task("x", "APPROVE", "2026-06-15T00:00:00+00:00"),
          subject_identifier: "SOP-PUR-014",
          subject_title: "Supplier Selection",
        },
      ]),
    ),
  );
...
it("shows a calm error state when the read fails", async () => {
  server.use(
    http.get("/api/v1/tasks", () => HttpResponse.json({ code: "error" }, { status: 500 })),
  );
  renderWithProviders(<MyTasksRail />);
