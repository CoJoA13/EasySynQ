...
import type { InterestedPartyRegisterState } from "../../lib/types";
import { PublishRegisterModal } from "./PublishRegisterModal";
import { useReleaseInterestedPartyRegister, useStartInterestedPartyRevision } from "./mutations";
...
// approve/decide step is a separate persona's job in the /tasks inbox (an IPR version is a DOCUMENT
// task) so it's not surfaced here.
//
// Gating is STATE × permission, both from the SERVER-computed caps on GET /interested-parties/register
// (the IPR head has no per-entity `capabilities` block): canManage = register.manage @ SYSTEM
// (start-revision / publish), canRelease = document.release over the head's multi-axis release scope
// (the faithful gate — a single-axis FE probe can't replicate artifact + folder + level + SoD-2;
// S-interested-parties-fe). The console renders only for a steward (quiet absence for everyone else);
// each button appears only when its state allows the act (never a dead/disabled affordance).
//
// We DON'T gate Publish on a client row count: GET /interested-parties is register.read-filtered, so a
// manage-without-read steward would see 0 parties for a non-empty register. The server's empty-register
...
export function RegisterLifecyclePanel({ state, canManage, canRelease }: Props) {
  const startRevision = useStartInterestedPartyRevision();
    ...
    <Card withBorder mt="md">
      <Stack gap="sm">
        ...
        {canManage && editable && (
          <Group justify="space-between" wrap="nowrap" gap="md">
            <Text size="xs" c="dimmed">
              Freezes the interested parties into a new version and submits it for approval.
            </Text>
        ...
        {canManage && state === "Effective" && (
          <Group justify="space-between" wrap="nowrap" gap="md">
            <Text size="xs" c="dimmed">
              Opens the next revision so interested parties become editable again.
            </Text>
            <Button
              variant="default"
...
function ReleaseConfirm({ opened, onClose }: { opened: boolean; onClose: () => void }) {
  const release = useReleaseInterestedPartyRegister();
  return (
    <ConfirmDestructive
      opened={opened}
      onCancel={onClose}
      ...
      title="Release the interested-parties register"
      consequence="This promotes the approved version to Effective — it becomes the governing read-of-record and the working parties lock until the next revision."
