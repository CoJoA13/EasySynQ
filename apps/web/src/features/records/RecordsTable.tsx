...
export function RecordsTable({ records }: { records: RecordSummary[] }) {
  ...
  return <Table.ScrollContainer minWidth={840}><Table striped highlightOnHover mt="md"><Table.Thead><Table.Tr>
    ...
    <Table.Td><Anchor component={Link} c="var(--es-accent-text)" to={`/records/${record.id}`} state={{ from: `${location.pathname}${location.search}` }} data-rownav aria-label={`Open record ${record.identifier ?? record.id}`}>{record.identifier ?? "Record"}</Anchor></Table.Td>
