...
import { usePublishContextRegister } from "./mutations";
...
// Publish a context-register revision — freeze the working issues into a new version and submit it for
// approval. The change reason is OPTIONAL (the server defaults a system reason when omitted); it's
// recorded on the frozen version + shown to the approver.
//
// Parent renders {open && <PublishRegisterModal/>} so close unmounts + resets the draft (the
// persistently-mounted-modal trap). On a thrown ApiError (e.g. a 409 "no issues to publish" / "not
// editable") the Alert surfaces it and the modal STAYS OPEN.
export function PublishRegisterModal({ opened, onClose }: Props) {
  const publish = usePublishContextRegister();
  ...
  return (
    ...
    >
      <Stack gap="sm">
        ...
        <Text size="sm">
          Freezes all context issues into a new version, then submits it for approval.
        </Text>
        <Textarea
          label="Change reason"
          description="Optional — recorded on the version and shown to the approver."
          placeholder="e.g. Annual review of internal and external issues"
          value={reason}
