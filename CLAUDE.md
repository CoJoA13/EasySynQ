...
- 2026-06-09 — **S-web-7c (Complaint & NCR intake) is FRONT-END ONLY** (no migration [head stays `0044`]/key/contract) — surfaces the already-built `/complaints*` + `/ncrs*` surface (`api/capa.py:493-617`) as **tabbed sub-routes under `/capa`**: a thin **`CapaLayout`** (Mantine `Tabs` Board·Complaints·NCRs + `<Outlet/>`, active from `useLocation().pathname`) wraps three child routes — `/capa` index = **`CapaBoardPage`** (its own title; the layout adds NO title; the board's existing flows are unchanged, it gained only a `?capa=<id>` deep-link seam — below), `/capa/complaints`, `/capa/ncrs`. **Per-key gating DIVERGES from the board:** complaints ride `record.read`/`record.create`/spawn=`capa.create`; NCRs ride `ncr.read`/`ncr.create`/`ncr.record_correction` — the **`demo` admin holds NONE** (both tabs calm-403), and **`ncr.create`+`ncr.record_correction` are seeded but granted to NO role** (0004) → **SYSTEM-override-only in v1** (gate each affordance on its OWN key at SYSTEM scope; never assume the admin sees it). **Spawn-CAPA idempotency is surfaced as STATE, not the HTTP status:** `api.send` discards the status (201-new vs 200-replay), so the `_complaint` row's **`spawned_capa_id`** drives the "Spawn CAPA"→"View CAPA" flip (invalidate `["complaints"]`+`["capas"]`); a racing replay just resolves. **Disposition is one-shot:** a disposed NCR row is **read-only with NO action button** (structural in the UI) + `409 ncr_already_dispositioned` is calm. **Spawn opens a `SpawnCapaModal` severity-confirm** (pre-filled from the complaint, REQUIRED) — a CAPA needs a severity (the backend 422s without one — `spawn_capa_from_complaint` resolves request-severity-preferred-as-late-triage `service.py:227/244`) and a complaint can be logged severity-less, so a **silent one-click inherit DEAD-ENDS** (422 behind a generic "try again") — the **pre-merge live smoke caught exactly this** (log a severity-less complaint → spawn fails); fixed to the confirm modal. No process picker (the 7b deferred Raise-scope decision); filters/tiles deferred (YAGNI). Fixtures pinned to the real `_complaint`/`_ncr` serializers (NOT the mockup). Live smoke: grant `demo` SYSTEM overrides of `record.read record.create capa.create ncr.read ncr.create ncr.record_correction` (org **AHT**) — one admin drives the loop (no SoD); the smoke also validated the real App.tsx→AppShell→CapaLayout wiring + per-key calm-403 panels (which unit tests bypass). **Codex P2s:** the NCR **disposition mutation invalidates on SETTLE not just success** (a 409-race refetch flips the row to read-only behind the calm error); the disposed NCR row shows the one-shot **who/when** audit (authorizer via `useUserDirectory`, degrade-to-id) since the table is the only NCR surface; **deep-link "View CAPA"** is implemented — `CapaBoardPage` opens its detail drawer from a **`?capa=<id>` URL param** (the S-web-4 `?from=&to=` pattern; the board's local card/raise opens stay URL-untouched + `closeDrawer` clears the param via `replace`, so existing flows + history are unchanged). **+35 web → 429 web tests** green.
