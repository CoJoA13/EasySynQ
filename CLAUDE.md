...
- `packages/contracts/openapi.yaml` — the living API contract; its executable toolchain is frozen by `packages/contracts/package-lock.json`, and `scripts/gen-contracts.sh` lints/bundles it and generates the Pydantic server models + TypeScript types. Document new endpoints in-PR.
...
- 2026-08-07 — **S-upload-identity = every non-dedup staging promotion carries one opaque, version-bound source identity across browser document check-in, Record capture/correction, generated content, and import workers (API + web + OpenAPI + Compose/operator docs; NO migration [head remains `0085`]; NO new permission key):** server-side streaming GETs and verifies the exact source version before copying it, then HEADs/verifies the returned target version without persisting that target identity in ORM/migrations/audit; rejection evidence commits after owner rollback and before exact-version cleanup, and public `409 staged_source_unavailable`, `422 staging_version_required`/`upload_identity_mismatch`, and `503 storage_unavailable` responses remain privacy-bounded. Community MinIO's owner-approved API-wide exact-origin CORS waiver exposes the browser's `x-amz-version-id` as response visibility only—presigned signature, method, key, expiry, and IAM remain authorization—and per-bucket CORS is unsupported by the pinned Community image. Four named mutations—digest acceptance, source-latest substitution, rolled-back audit evidence, and latest-key cleanup—each turned the intended unit/real-storage/domain proof RED and returned GREEN after restoration. Task 10 also repaired selector blind spots and stale test consumers without changing production behavior. **Fresh gates:** API Ruff/format/mypy + 1,686 unit; web 249 files/1,468 tests + lint/typecheck/build; 283 contract; 1,051 integration passed (2 skipped); one-container PG16 `upgrade head → downgrade base → upgrade head → alembic check` clean; head `0085`; Redocly/generator; R61 and corrected scope audit green, including zero migration matches for target-version persistence. (Task 10 verification repairs `e6f531d`; shipped evidence recorded here and in `docs/slice-history.md`.)
