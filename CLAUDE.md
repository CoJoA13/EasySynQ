...
- 2026-06-08 — **Native-Windows dev gotchas (this box):** run **`uv` via PowerShell, not Git Bash** — MSYS rewrites uv's managed-Python version-link target to `/c/...` → `error: Missing expected target directory for Python minor version link`; PowerShell `uv run` resolves it (syncs + runs Python 3.12.13). **`-m integration` CANNOT run on native Windows** — psycopg-async rejects the Windows `ProactorEventLoop`, so EVERY integration test fails at the DB connect → integration is a **Linux-CI-only gate** here (`just check` fast-loops is the local gate, by design). mypy also flags `os.O_NOFOLLOW` (Unix-only) in `ingestion/source.py` on Windows — pre-existing, green on Linux.
