...
Review & Approve page [redline + decision card] · the doc-page **Approvals stepper** · **Release**). **S-web-6 = Global
Search + Compliance Checklist — implemented, PR open**: **FRONT-END ONLY** (no migration/key/`openapi.yaml` — surfaces the
**S10** backend) — a hand-rolled **⌘K command palette** + a `/search?q=` ranked results page (XSS-safe `ts_headline`
snippet · `hidden_by_scope` footer · filters-not-403) + a permission-gated `/compliance` **Compliance Checklist** (20★
coverage, RAG rollup, clause drill-through; HARD-gated `report.compliance_checklist.read` → 403-calm for the demo admin).
**162 web tests**; subagent-driven TDD. Also open: the v1.x drift family (D1–D5). **Migration head `0044` (next `0045`).**
