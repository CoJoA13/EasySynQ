...
- `apps/web/` — React/TS + Mantine + Tailwind SPA. The web-UI track is feature-complete (~943 tests; per-slice list in **Current status** + `docs/slice-history.md`). Stack-free tests: vitest + MSW + jest-axe (`npm test`); under `src/`: `app/shell/` · `features/` · `lib/` · `theme/` · `test/`.
...
- 2026-06-14 — Reconcile server↔draft editor state by **keying the editor** on `` `${id}::${contentSignature}` `` to remount-and-reseed (never a render-sync effect) + write the mutation response straight into cache (`setQueryData`) to close the stale-refetch window (#132).
...
- 2026-06-13 — ⚠ On this native-Windows box, `pytest -m integration` **and** the full unit suite both fail (ProactorEventLoop / native crash) → integration + full-unit are **CI-ONLY**; verify locally via ruff + mypy-strict + **targeted** unit tests (S-mr-2).

## Current status

> **MVP COMPLETE** (S0–S11). **v1 feature-complete** — every family shipped end-to-end: Records & evidence · Ingestion · Audits/Findings/CAPA · Revision/change-depth (**DCR** — read + write + diff + annotate + CREATE-implement; no residuals) · Drift **D1–D5** · Acknowledgements · Quality Objectives (lifecycle/revision + KPI trend chart) · Management Review (backend + UI + outputs→action-systems + filed-minutes pack) — so the **ISO 9001:2015 ★ spine is complete**. The full **web-UI track is feature-complete** (~962 web tests). **Migration head `0051` (next `0052`).** Per-slice changelog + named deferred residuals: `docs/slice-history.md`.
