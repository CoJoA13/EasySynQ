...
drawer + the `_capa` `title`/`created_at`/`raised_by` enrichment); **S-web-7b** = CAPA **lifecycle writes** (this PR) —
...
— no migration/key). **395 web tests**; subagent-driven TDD; diff-critic caught a looped-CAPA duplicate-label linker (fixed
to current-cycle-only). Still open: **S-web-7c** (complaints/NCR intake) + **S-web-7d** (audits/findings) of the epic; the
v1.x drift family (D1–D5); the PDCA dashboard (deferred until acks/objectives land). **Migration head `0044` (next `0045`)
— 7b added no migration.** Full per-slice narrative + deferred residuals: `docs/slice-history.md`.
