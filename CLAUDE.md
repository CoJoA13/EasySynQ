...
`hidden_by_scope` · filters-not-403] + a gated `/compliance` 20★ Checklist). **S-ing-4b = Ingestion Review UI — CLOSES UJ-2
(PR open)**: **FRONT-END ONLY** (no migration/key/`openapi.yaml` — surfaces the S-ing-1..5 `/admin/imports/*` surface) — a
four-faces run page (scan-progress → review cockpit → commit-progress → terminal+resume), a runs landing + New-Import, and
the review cockpit (4 tiles · 5 queue tabs · **virtualization-free** paginated triage table · multi-select bulk +
Bulk-accept-High · R10 kind-confirm a separate act · merge/split · pre-commit checklist · "Commit N confirmed"). **318 web
tests**; subagent-driven TDD; diff-critic caught the `run.counts`-is-flat (not `queues`/`review`-namespaced) false-PASS +
the resume-wiring gap (both fixed). The `demo` admin holds all three import keys, so it drives the whole loop. Also open:
the v1.x drift family (D1–D5), the CAPA board, the PDCA dashboard (deferred until acks/objectives land). **Migration head
`0044` (next `0045`).** Full per-slice narrative + deferred residuals: `docs/slice-history.md`.
