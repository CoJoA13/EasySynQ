...
- **Code:** building the MVP foundation-first, slice by slice (each via a PR with green CI on protected `main`).
  ...
  - **S9 — clause IA + `clause_mapping`** ✅ — the read-only ISO 9001:2015 clause spine (an 83-clause seeded catalog, the 20 ★ mandatory documented-information items) + the M:N document↔clause mapping (`GET /clauses`, flat `…/clause-mappings` sub-resources) + the lifecycle submit gate (a document needs ≥1 clause mapping before review). Owner-scoped to clause IA; process IA + the clause/process mirror tree are **S9b**.
  - **S9b — clause/process mirror tree + process IA** (next) — rebuild the mirror into the §10.3 PLAN/DO/CHECK/ACT→clause tree (now unblocked) + the process map (`process`/`process_edge`/`process_link`, `GET /processes`); couples to the deferred wizard scope/process-map step.
