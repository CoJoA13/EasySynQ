...
- `just check` — the **fast** local gate (ruff + mypy-strict + targeted unit + the web loop; no
  Docker, and **NOT the full suites**). ⚠ The **full unit suite AND `pytest -m integration` CRASH
  locally on this native-Windows box** (ProactorEventLoop / native crash) → both are **CI-only**;
  verify them in CI, never locally. Known baseline: **17 expected unit failures in exactly 3 files**
  (mirror symlinks ×12, ingestion `O_NOFOLLOW` ×5) — anything outside those is a real regression.
