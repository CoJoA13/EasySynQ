...
- **Integration-test traps (where the false-PASS hides):**
  ...
  - ⚠ Do not recommend one shared-DB local `-m integration` run as the sole gate. Run the scoped file plus a cross-file ordering proof; the isolated CI shards are authoritative (`docs/dev-workflow.md`).
